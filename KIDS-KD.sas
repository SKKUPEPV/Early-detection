/*Raw data*/
libname a "E:\연구실컴퓨터\바탕화면\Early detection\rawdata";
...
proc import out = system_organ_feature
	datafile = 'D:\연구\Early detection\WHO-ART092-MedDRA21.0 Bridge-6 July 2018'
	DBMS = xlsx replace;
run;

data system_organ_feature;
	set system_organ_feature;
	arrn_1 = arrn*1;
run;
...
proc import out = label
	datafile = 'D:\연구\Early detection\gold_standard_whoart'
	DBMS = xlsx replace;
	sheet = 'infliximab';
run;
