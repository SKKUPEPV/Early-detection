...
/*Generate the 10-year cumulative dabase*/
	%do year = 2009 %to 2018;

	data cumulative_DB_&year;
		set analysis_study;
		if 2009 <= year <= &year;
	run;
...
data statistical_feature_&year;
	set stat_feature_preprocessing_&year;
	if c=. then c = 0;
	if d=. then d = m1;
	drop m1;
run;
...
data cov_feature_preprocessing_&year;
	set cumulative_DB_&year;
	...
	%covariate(rpt_csf_num,0);
	%covariate(rpt_csf_num,1);
	%covariate(rpt_csf_num,2);
	%covariate(rpt_csf_num,3);
	%covariate(rpt_csf_num,4);
	%covariate(rpt_csf_num,5);
